...
import io.github.mbarcia.csv.repository.CsvPaymentsInputFileRepository;
...
class PersistCsvPaymentsInputFileReactiveServiceTest {
  ...
  void setUp() {
    ...
    persistCsvPaymentsInputFileReactiveService =
        new PersistCsvPaymentsInputFileReactiveService(repository);
    File testFile = new File("/test/input.csv");
    testInputFile = new CsvPaymentsInputFile(testFile);

    // Mock the repository to return the same object
    when(repository.persist(any(CsvPaymentsInputFile.class)))
        .thenReturn(Uni.createFrom().item(testInputFile));
  }
  ...
  void testProcess() {
    ...
    try (MockedStatic<MDC> mdcMock = mockStatic(MDC.class)) {
      ...
      try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
        ...
        Uni<CsvPaymentsInputFile> result =
            persistCsvPaymentsInputFileReactiveService.process(testInputFile);

        // Verify the result
        UniAssertSubscriber<CsvPaymentsInputFile> subscriber =
            result.subscribe().withSubscriber(UniAssertSubscriber.create());
        subscriber.assertCompleted().assertItem(testInputFile);

        // Verify repository interaction
        verify(repository).persist(testInputFile);

        // Verify MDC interactions
        mdcMock.verify(
            () ->
                MDC.put("serviceId", PersistCsvPaymentsInputFileReactiveService.class.toString()));
