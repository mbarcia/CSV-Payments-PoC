...
class SendPaymentRecordCommandTest {
    ...
    @Mock
    private PaymentProviderServiceMock paymentProviderServiceMock;
    ...
    void execute_happyPath_shouldReturnAckPaymentSent() {
        ...
        paymentRecord.setId(UUID.randomUUID()); // Set the ID as it's generated in BaseEntity's constructor
        ...
        when(paymentProviderServiceMock.sendPayment(any(SendPaymentRequestMapper.SendPaymentRequest.class)))
        ...
        ArgumentCaptor<SendPaymentRequestMapper.SendPaymentRequest> requestCaptor = ArgumentCaptor.forClass(SendPaymentRequestMapper.SendPaymentRequest.class);
    ...
    void execute_unhappyPath_shouldPropagateException() {
        ...
        // Then
        RuntimeException thrown = assertThrows(RuntimeException.class, () -> sendPaymentRecordCommand.execute(paymentRecord).await().indefinitely());
        assertThat(thrown).isEqualTo(expectedException);

        verify(paymentProviderServiceMock, times(1)).sendPayment(any(SendPaymentRequestMapper.SendPaymentRequest.class));
    }
}
