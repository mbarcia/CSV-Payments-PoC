...
class PaymentProviderServiceMockTest {
    ...
    void sendPayment_happyPath_shouldReturnAckPaymentSent() {
        // Given
        PaymentRecord paymentRecord = new PaymentRecord("csvId-" + UUID.randomUUID(), "John Doe", new BigDecimal("100.00"), java.util.Currency.getInstance("USD"));
        ...
        PaymentProviderServiceMock paymentProviderServiceMock = new PaymentProviderServiceMock(
                this.ackPaymentSentMapper,
                this.paymentStatusMapper,
                config
        );

        // When
        AckPaymentSent testAckPaymentSent = new AckPaymentSent(UUID.randomUUID())
                .setStatus(1000L)
    ...
    void sendPayment_throttled_shouldThrowException() throws InterruptedException {
        ...
        PaymentProviderServiceMock paymentProviderServiceMock = new PaymentProviderServiceMock(ackPaymentSentMapper, paymentStatusMapper, config);
        ...
        paymentProviderServiceMock.sendPayment(new SendPaymentRequestMapper.SendPaymentRequest()
        ...
        // When & Then
        StatusRuntimeException thrown = assertThrows(StatusRuntimeException.class, () -> paymentProviderServiceMock.sendPayment(request));
        assertThat(thrown.getStatus().getCode()).isEqualTo(Status.Code.RESOURCE_EXHAUSTED);
        assertThat(thrown.getStatus().getDescription()).isEqualTo("Payment service is currently throttled. Please try again later.");
    }
    ...
    void sendPayment_timeoutMinusOne_shouldThrowException() {
        ...
        this.paymentProviderServiceMock = new PaymentProviderServiceMock(ackPaymentSentMapper, paymentStatusMapper, config);
    ...
    void getPaymentStatus_happyPath_shouldReturnPaymentStatus() {
        ...
        PaymentStatus testPaymentStatus = new PaymentStatus()
            .setReference("101")
            .setStatus("nada")
    ...
    void getPaymentStatus_throttled_shouldThrowException() throws InterruptedException {
        ...
        // When & Then
        StatusRuntimeException thrown = assertThrows(StatusRuntimeException.class, () -> paymentProviderServiceMock.getPaymentStatus(testAckPaymentSent));
        assertThat(thrown.getStatus().getCode()).isEqualTo(Status.Code.RESOURCE_EXHAUSTED);
        assertThat(thrown.getStatus().getDescription()).isEqualTo("Failed to acquire permit within timeout period. The payment status service is currently throttled.");
    }

    static class FakePaymentProviderConfig implements PaymentProviderConfig {
        @Override public double permitsPerSecond() { return 1000.0; }
        @Override public long timeoutMillis() { return 5000; }
        @Override public double waitMilliseconds() { return 50.0; }
    }

    private static class ThrottledPaymentProviderConfig implements PaymentProviderConfig {
        @Override public double permitsPerSecond() { return 0.001; }
        @Override public long timeoutMillis() { return 5000; }
        @Override public double waitMilliseconds() { return 50.0; }
    }

    private static class NegativeTimeoutPaymentProviderConfig implements PaymentProviderConfig {
        @Override public double permitsPerSecond() { return 100.0; }
        @Override public long timeoutMillis() { return -1L; }
        @Override public double waitMilliseconds() { return 50.0; }
    }

    private static class LowRatePaymentProviderConfig implements PaymentProviderConfig {
        @Override public double permitsPerSecond() { return 0.001; }
        @Override public long timeoutMillis() { return 0; }
        @Override public double waitMilliseconds() { return 50.0; }
    }
