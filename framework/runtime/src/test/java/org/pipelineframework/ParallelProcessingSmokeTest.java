...
class ParallelProcessingSmokeTest {
    ...
    void testParallelProcessingWorks() {
        ...
        // Since each operation takes 100ms, if they start at similar times, they're likely executing in parallel
        ...
        for (int i = 0; i < executionTimestamps.size(); i++) {
            ...
            for (int j = i + 1; j < executionTimestamps.size(); j++) {
                ...
                // If execution times are within the threshold of each other, consider them concurrent
        ...
        // (3) Timing assertion: total duration should be significantly less than sum of individual delays
        // Since each item takes ~100ms sequentially, 3 items would take ~300ms sequentially, 
        // but in parallel they should take closer to ~100ms (though we allow more for test overhead)
        long totalDuration = endTime - startTime;
        
        // If we don't have concurrent execution on the same thread, at least verify that the results are correct
        if (!hasConcurrentExecution) {
            System.out.println("Note: Could not verify concurrent execution, but checking timing. Timestamps: " + executionTimestamps + 
                              ", Threads: " + executionThreads);
            // Still check timing as evidence of parallelism
            hasConcurrentExecution = totalDuration < 250; // Sequential would take ~300ms, parallel should be faster
        }
        
        assertTrue(hasConcurrentExecution, 
            String.format("Items should execute concurrently (either via overlapping timestamps or different threads). " +
                "Timestamps: %s, Threads: %s, Duration: %d ms", executionTimestamps, executionThreads, totalDuration));
    }
