...
class ProcessCsvPaymentsInputFileGrpcServiceTest {
    ...
    @InjectMocks
    ProcessCsvPaymentsInputFileGrpcService grpcService;
    ...
    void remoteProcess() {
        // Given
        InputCsvFileProcessingSvc.CsvPaymentsInputFile grpcRequest = InputCsvFileProcessingSvc.CsvPaymentsInputFile.newBuilder()
                .setFilepath("/tmp/test.csv")
                .build();

        InputCsvFileProcessingSvc.PaymentRecord grpcOutput = InputCsvFileProcessingSvc.PaymentRecord.newBuilder().build();

        CsvPaymentsInputFile domainInputFile = new CsvPaymentsInputFile(new java.io.File("/tmp/test.csv"));

        PaymentRecord domainPaymentRecord = new PaymentRecord(
        ...
        // When
        when(csvPaymentsInputFileMapper.fromGrpc(any(InputCsvFileProcessingSvc.CsvPaymentsInputFile.class))).thenReturn(domainInputFile);
        when(domainService.process(any(CsvPaymentsInputFile.class))).thenReturn(Multi.createFrom().item(domainPaymentRecord));
        when(paymentRecordMapper.toGrpc(any(PaymentRecord.class))).thenReturn(grpcOutput);

        InputCsvFileProcessingSvc.PaymentRecord grpcPaymentRecord = InputCsvFileProcessingSvc.PaymentRecord.newBuilder()
        ...
        when(csvPaymentsInputFileMapper.fromGrpc(any(InputCsvFileProcessingSvc.CsvPaymentsInputFile.class)))
        ...
        when(paymentRecordMapper.toDto(any(PaymentRecord.class)))
                .thenReturn(com.example.poc.common.dto.PaymentRecordDto.builder()
        ...
        Multi<InputCsvFileProcessingSvc.PaymentRecord> resultMulti = grpcService.remoteProcess(grpcRequest);

        // Then
        AssertSubscriber<InputCsvFileProcessingSvc.PaymentRecord> subscriber = resultMulti.subscribe().withSubscriber(AssertSubscriber.create(1));
